...
class EndpointFuzzer(BaseModule):
    ...
    def run(self) -> list:
        ...
        try:
            timeout  = max(1, int(self.get_option("TIMEOUT") or 8))
            threads  = min(200, max(1, int(self.get_option("THREADS") or 20)))
            delay    = max(0, int(self.get_option("DELAY") or 0))
        except ValueError:
            print_status("TIMEOUT, THREADS and DELAY must be numeric.", "error")
            return []
        auth     = self.get_option("AUTH") or ""
        try:
            codes = set(int(c.strip()) for c in (self.get_option("CODES") or "200").split(",") if c.strip())
        except ValueError:
            print_status("CODES must be a comma-separated list of HTTP status codes.", "error")
            return []
        ...
        if auth:
            # NOTE: .lstrip("Bearer ") strips a *set of characters*, not a
            # prefix, so a token starting with "B", "e", "a", "r" or a
            # space (very common in base64/JWT tokens) was getting its
            # leading characters silently eaten. removeprefix() (3.9+) is
            # the correct way to strip a literal prefix.
        ...
        def probe(combo):
            ...
            if code in codes:
                ...
                with lock:
                    count += 1
                    if count == 1:
                        print(f"  {Colors.DARK_GRAY}{'PROG':>9}  {'CODE':<5} {'SEV':<7} {'METHOD':<7} ENDPOINT{Colors.RESET}")
                    print(f"  {Colors.DARK_GRAY}[{count:>5}/{total}]{Colors.RESET}  "
                          f"{_code_color(code)}{code:<5}{Colors.RESET} "
                          f"{sev:<7} "
